...
function loadTagihan(){
  ...
  db.collection('tagihan').where('uid','==',uid).get()
    .then(snap => {
      ...
      riwayatEl.innerHTML = '';
      if(snap.empty){
        riwayatEl.innerHTML = '<p class="empty-state">Belum ada data tagihan. Admin belum membuat tagihan bulan ini.</p>';
...
function loadSurat(){
  ...
  db.collection('surat').where('uid','==',uid).get().then(snap => {
    ...
    docs.forEach(doc => {
      const d = doc.data();
      const pillClass = d.status === 'Disetujui' ? 'pill-green' : (d.status === 'Ditolak' ? 'pill-red' : 'pill-red');
      el.innerHTML += `
        <div class="list-item">
  ...
  db.collection('jenisTagihan').get().then(snap => {
    ...
    document.querySelectorAll('[data-del-jenis]').forEach(icon => {
      icon.addEventListener('click', () => {
        db.collection('jenisTagihan').doc(icon.getAttribute('data-del-jenis')).delete().then(loadAdminTagihan).catch(err => alert('Gagal menghapus: ' + err.message));
      });
    });
...
function loadAdminInformasi(){
  db.collection('informasi').orderBy('tanggal','desc').get().then(snap => {
    ...
    document.querySelectorAll('[data-del-info]').forEach(icon => {
      icon.addEventListener('click', () => {
        db.collection('informasi').doc(icon.getAttribute('data-del-info')).delete().then(loadAdminInformasi).catch(err => alert('Gagal menghapus: ' + err.message));
      });
    });
  }).catch(err => tampilkanErrorMuat('ai-list', err));
}
...
document.getElementById('form-admin-informasi').addEventListener('submit', e => {
  ...
  db.collection('informasi').add({
    ...
    loadAdminInformasi();
  }).catch(err => tampilkanErrorSimpan('ai-error', err));
});
...
function loadAdminSurat(){
    ...
    document.querySelectorAll('[data-acc]').forEach(btn => {
      btn.addEventListener('click', () => {
        db.collection('surat').doc(btn.getAttribute('data-acc')).update({ status: 'Disetujui' })
          .then(() => { loadAdminSurat(); loadAdminBadges(); })
  ...
  }).catch(err => tampilkanErrorMuat('as-list', err));
}
  ...
  db.collection('laporan').orderBy('dibuatPada','desc').limit(30).get().then(snap => {
    ...
    document.querySelectorAll('[data-status-id]').forEach(sel => {
      sel.addEventListener('change', () => {
        const nilaiSebelumnya = sel.getAttribute('data-current') || sel.value;
        db.collection('laporan').doc(sel.getAttribute('data-status-id')).update({ status: sel.value })
          .then(loadAdminBadges)
          .catch(err => { alert('Gagal menyimpan status: ' + err.message); sel.value = nilaiSebelumnya; });
      });
    });
  }).catch(err => tampilkanErrorMuat('al-list', err));
}
...
function loadAdminCctv(){
  db.collection('cctv').get().then(snap => {
    ...
    document.querySelectorAll('[data-del-cctv]').forEach(icon => {
      icon.addEventListener('click', () => {
        db.collection('cctv').doc(icon.getAttribute('data-del-cctv')).delete().then(loadAdminCctv).catch(err => alert('Gagal menghapus: ' + err.message));
      });
    });
  }).catch(err => tampilkanErrorMuat('ac-list', err));
}
...
document.getElementById('form-admin-cctv').addEventListener('submit', e => {
  ...
  db.collection('cctv').add({ lokasi, link, status: 'Aktif' }).then(() => {
    ...
    loadAdminCctv();
  }).catch(err => tampilkanErrorSimpan('ac-error', err));
});
...
function loadAdminKontak(){
  db.collection('kontakDarurat').get().then(snap => {
    ...
    document.querySelectorAll('[data-del-kontak]').forEach(icon => {
      icon.addEventListener('click', () => {
        db.collection('kontakDarurat').doc(icon.getAttribute('data-del-kontak')).delete().then(loadAdminKontak).catch(err => alert('Gagal menghapus: ' + err.message));
      });
    });
  }).catch(err => tampilkanErrorMuat('ak-list', err));
}
...
document.getElementById('form-admin-kontak').addEventListener('submit', e => {
  ...
  db.collection('kontakDarurat').add({ nama, nomor, darurat: true }).then(() => {
    ...
    loadAdminKontak();
  }).catch(err => tampilkanErrorSimpan('ak-error', err));
});
...
function loadAdminKas(){
  db.collection('kas').orderBy('tanggal','desc').get().then(snap => {
    ...
    tampilkanErrorMuat('akas-masuk-list', err);
    tampilkanErrorMuat('akas-keluar-list', err);
  });
}
...
document.getElementById('form-kas-masuk').addEventListener('submit', e => {
  ...
  db.collection('kas').add({
    ...
    loadAdminKas();
  }).catch(err => tampilkanErrorSimpan('akm-error', err));
});
...
document.getElementById('form-kas-keluar').addEventListener('submit', e => {
  ...
  batch.commit().then(() => {
    ...
    loadAdminKas();
  }).catch(err => tampilkanErrorSimpan('akk-error', err));
});
...
document.getElementById('form-profil-admin').addEventListener('submit', e => {
  ...
  errorEl.textContent = '';
  db.collection('pengaturan').doc('kontakAdmin').set({ nama, hp, email }, { merge: true }).then(() => {
    const successEl = document.getElementById('profil-admin-success');
    successEl.textContent = 'Berhasil tersimpan!';
    setTimeout(() => { successEl.textContent = ''; }, 2500);
  }).catch(err => tampilkanErrorSimpan('profil-admin-error', err));
});
...
/* =========================================================
   24. PWA: TOMBOL INSTAL DI HALAMAN DAFTAR
========================================================= */
